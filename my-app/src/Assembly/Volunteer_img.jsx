//志愿者招募
import '../CSS/Volunteer_img.css'
import React, { Component } from 'react'
export default class Volunteer_img extends Component {

    render() {
        return (
            <div>
                <div>
                    <hr />
                    <div className="xingjiaoyu">
                        <h1 id="yuanqi">性教育守护流动儿童</h1>
                    </div>
                    <div>
                        <h1 id="yuanqi">项目缘起</h1>
                        <hr />
                        <br />
                        <div className="sikao">
                            <div className="Center">
                                <h3>“一起发生在打工子弟学校的性侵事件”</h3>
                            </div>
                            <div>
                                <h3>并非“偶发”、“个案”</h3>
                                <li>不知道</li>
                                <li>不报警</li>
                                <li>不求助</li>
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1>思考</h1>
                        <hr />
                        <br />
                        <div className="sikao">
                            <li>
                                我是谁？
                            </li>
                            <li>我为什么想要去为小学生讲性教育课</li>
                            <li>我计划做多久？</li>
                            <li>我自己希望收获什么？</li>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1 id="jieshao">希希学园介绍</h1>
                        <hr />
                        <br />
                        <div className="Erect">
                            <div className="Beijing Fensan">
                                <h2>背景</h2>
                            </div>
                            <div className="Fensan Beijing_2">
                                <div>
                                    <div></div>
                                    <div>教育资源</div>
                                    <div>学校无意愿、无课程</div>
                                    <div>家长无意识、无能力</div>
                                </div>
                                <div>
                                    <div></div>
                                    <div>流动儿童</div>
                                    <div>有接受性教育的权利</div>
                                    <div>有学习性知识的渴望</div>
                                    <div>有遭受性侵害的危险</div>
                                </div>
                            </div>

                            <div>
                                <br />
                                <br />
                                <hr />
                                <br />
                                <br />
                                <div className="xuexiao">
                                    <div>
                                        <h1>学校教育</h1>
                                        <div>北京希希儿童关爱中心目前已与近 20 所学校合作，开设全面性教育课程。
                                            课程符合联合国教科文组织《全面性教育技术指导纲要》及教育部《中小学健
                                            康教育指导纲要》的要求。课程涵盖 1-6 年级 12 学期，每学期 6 节课。不同
                                            年级主题不同，内容前后衔接，循序渐进，学生在不同年龄段接受适合的性健
                                            康知识。在小学阶段，通过 72 节课程，给孩子全面性教育。
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <hr />
                            <div className="kecheng">
                                <div>
                                    <div>
                                        <div>人体发育</div>
                                        <div>性健康行为</div>
                                        <div>性与生殖健康</div>
                                    </div>
                                    <div>
                                        <div>
                                            性教育
                                        </div>
                                        <div>
                                            <div>课</div>
                                            <div>程</div>
                                        </div>
                                    </div>
                                    <div>
                                        <div>人际关系</div>
                                        <div>价值观、态度与技能</div>
                                        <div>文化、社会与权力</div>
                                    </div>
                                </div>
                            </div>
                            <hr />
                            <div className="shequ">
                                <div>
                                    <h1>社区教育</h1>
                                    <div className="sikao">
                                        <li>全面性教育系列课程（针对不同年龄段儿童，每期 10 次课</li>
                                        <li>专题讲座（“青春期知识讲座”、“认识身体，学会自我保护”等）</li>
                                        <li>家长课堂（讲座、读书会）</li>
                                        <li>亲子游园会</li>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1 id="ertong">谁是流动儿童?</h1>
                        <hr />
                        <br />
                        <div className="Erect shei">
                            <div>
                                <div className="Erect">
                                    <div className="Center">
                                        <div>“流动儿童”</div>
                                    </div>
                                    <div>
                                        <div>
                                            人口统计部门：流动儿童是指居住地与户口登记地所在的乡镇街
                                            道不一致且离开户口登记地半年以上，不包括市辖区内人户分离，
                                            年龄在18周岁以下的人口
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div className="Bold">“进城务工人员随迁子女”</div>
                            </div>
                            <div>
                                <div className="text Bold hukou">户口</div>
                            </div>
                            <div>
                                <div className="Erect">
                                    <div className="Center">
                                        <div>
                                            “随迁子女”
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            教育统计部门：随迁子女是指户籍登记在外省（区、市）、本省外县（区）
                                            ，随务工父母到输入地（同住）并在校接受教育的适龄儿童少年。
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div>
                                <div className="Erect">
                                    <div className="Center">
                                        <div>
                                            “打工子女”
                                        </div>
                                    </div>
                                    <div>
                                        <div>
                                            非正式称呼：基于传统城乡二元结构的假设，对于外出打工者子女们
                                            的一种习以为常的一种称谓，没有准确的定义。
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1 id="fazhan">机构发展历程</h1>
                        <hr />
                        <br />
                        <div className="fazhan Erect">
                            <div>
                                <div>
                                    <div>2014</div>
                                    <div>秋季学期，支持11所学校开设《珍爱生命》课程</div>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>2015</div>
                                    <div>作为新公民计划的孵化项目，开始独立运营</div>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>2016</div>
                                    <div>在北京市民政局正式注册为“民办非企业”北京希希儿童关爱中心</div>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>2017</div>
                                    <div>在15所学校、16个社区开展性教育活动</div>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>2018</div>
                                    <div>支持了13所幼儿园、学校性教育课程，在30多所学校开展了青春期知识讲座。组织性教育绘画比赛</div>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>2019</div>
                                    <div>支持11所打工子弟小学、1所幼儿园以及实务学堂性教育；承接政府购买服务项目，在东城崇西社区阅读活动</div>
                                </div>
                            </div>
                            <div>
                                <div>
                                    <div>2020</div>
                                    <div>尝试学生线上课程、志愿者线上培训、性教育绘本分享</div>
                                </div>
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1>愿景</h1>
                        <hr />
                        <br />
                        <div id="yuanjing" className="yuanjing">
                            <div>
                                每个孩子在成长过程中都可以接受到
                                适合其年龄、认知水平的全面性教育，
                                掌握科学、正确的性知识，
                                形成积极的性态度和价值观，
                                能够正确认识自己、
                                有效保护自己。
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1>理念</h1>
                        <hr />
                        <br />
                        <div className="linian">
                            <div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/Funeng.svg" alt="" />
                                    </div>
                                    <div>赋能</div>
                                </div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/Pingdeng.svg" alt="" />
                                    </div>
                                    <div>平等</div>
                                </div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/Zunzhong.svg" alt="" />
                                    </div>
                                    <div>尊重</div>
                                </div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/Xinren.svg" alt="" />
                                    </div>
                                    <div>信任</div>
                                </div>
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                    <div className="shiming_1">
                        <h1>使命</h1>
                        <hr />
                        <br />
                        <div className="shiming">
                            <div>
                                <div>
                                    <img src="Img_Linian/Shiming.png" alt="" />
                                </div>
                                <div>
                                    <div>通过支持学生教材、提供教师培训等方式在打工子弟学校推广全面性教育课程。</div>
                                    <div>通过社工培训等方式支持流动人口社区推广家庭性教育。</div>
                                </div>
                            </div>

                        </div>
                        <br />
                        <br />
                    </div>
                    <div>
                        <h1 id="jinzhan">项目产出</h1>
                        <hr />
                        <br />
                        <div className="chanchu">
                            <div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/chanchu (1).png" alt="" />
                                    </div>
                                    <div>
                                        <div>支持22所学校开设性教育课程</div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/chanchu (2).png" alt="" />
                                    </div>
                                    <div>
                                        <div>累计培训教师400多人</div>
                                    </div>
                                </div>
                                <div>
                                    <div>
                                        <img src="Img_Linian/chanchu (3).png" alt="" />
                                    </div>
                                    <div>
                                        <div>本学期服务流动儿童4072人</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <br />
                        <br />
                    </div>
                </div>
            </div>
        )
    }
}
